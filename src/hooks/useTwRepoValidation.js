...
export default function useTwRepoValidation({authentication, owner, server, languageId, refresh}) {
  const [{twRepoTree, 
    ...
    twRepoTreeStatus}, 
    setValues
  ] = useState({twRepoTree:null, twRepoTreeManifest:null, twManifestSha:null, twRepoTreeStatus:WAITING})
  ...
  useEffect(() => {
    async function getReposTrees() {
      ...
      setValues({twRepoTree: _tree, twRepoTreeManifest: _manifest, twManifestSha: _manifestSha, twRepoTreeStatus: _errorMesage})
    }
