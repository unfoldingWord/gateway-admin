...
export default function useObsSnRepoValidation({authentication, owner, server, languageId, refresh}) {
  const [{obsSnRepoTree, 
    ...
    setValues
  ] = useState({obsSnRepoTree:null, obsSnRepoTreeManifest:null, obsSnRepoTreeErrorMessage:WAITING})
  // Translation Notes Hook
  // Example: https://qa.door43.org/api/v1/repos/vi_gl/vi_sn/git/trees/master?recursive=true&per_page=99999
  useEffect(() => {
    async function getReposTrees() {
      setValues({obsSnRepoTree: null, obsSnRepoTreeManifest: null, obsSnRepoTreeErrorMessage: WORKING})
