...
export default function useTwlRepoValidation({authentication, owner, server, languageId, refresh}) {
  const [{twlRepoTree, 
    ...
    setValues
  ] = useState({twlRepoTree:null, twlRepoTreeManifest:null, twlRepoTreeErrorMessage:WAITING})
  // Translation Notes Hook
  // Example: https://qa.door43.org/api/v1/repos/vi_gl/vi_twl/git/trees/master?recursive=true&per_page=99999
  useEffect(() => {
    async function getReposTrees() {
      setValues({twlRepoTree: null, twlRepoTreeManifest: null, twlRepoTreeErrorMessage: WORKING})
